...
struct
{
    ...
    int end;
    int length;
}print_sorted[MCST];//由大而小印出來使用,只存放被選上的
...
int main(void)
{
    ...
    int print_node,print_smallest;
    //每次選出當前最小者
    int smallest=999,smallest_index=0;
    for(print_node=0;print_node<vertices_num-1;print_node++)
    {//一棵MCST最多有vertices_num-1個邊
        smallest=999;
        for(print_smallest=0;print_smallest<edges_num;print_smallest++)
        {
            if(selected[print_smallest]!=0)
            {
                if(smallest>=edges[print_smallest].length)
                {
                    smallest=edges[print_smallest].length;
                    smallest_index=print_smallest;
                }
            }
        }
        //現在是當次最小的了!
        selected[smallest_index]=0;//讓她下次不被選
        printf("(%d,%d)=%d\n",edges[smallest_index].node1,edges[smallest_index].node2,edges[smallest_index].length);
    }
    /*
